...
static int callback(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
	...
	//Wskaznik na bufor zawierający pakiet
    unsigned char *datagram;
	// Rozmiar komunikatu (naglowek + dane)
    int retval = nfq_get_payload(nfa, &datagram);
	// Wypelnienie naglowka icmp
    struct icmphdr *icmp_header = (struct icmphdr *)(datagram + sizeof(struct iphdr));
	// Wskaznik na tablice bajtow
    char *packet_data = (char* )(datagram + sizeof(struct icmphdr) + sizeof(struct iphdr));
	// Rozmiar tablicy
	...
	//Wypelnianie sumy kontrolnej
    icmp_header->checksum = 0;
    icmp_header->checksum = internet_checksum((unsigned short *) datagram, retval);
	
	//Przekazanie zmodyfikowanego pakietu i jego rozmiaru do fukcji nfq_set_verdict
    return nfq_set_verdict(qh, id, NF_ACCEPT, retval, datagram);
}
