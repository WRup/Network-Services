...
#include <libnetfilter_queue/libnetfilter_queue.h> /* Tutaj jest meta-naglowek zdefiniowany */

/* Funckja callback = analiza/modyfikacja pakietu i wydanie werdyktu
(Pakiet porzucony/akceptacja pakietu)
Argumenty: qh = uchwyt kolejki, nfmsg = struktura wiadomosci zawierajaca pakiet,
		   nfa = uchwyt danych, data = dane użytkownika
*/
...
int main(int argc, char **argv) {
     ...
     */
	 // Najpierw odwiazanie koolejki od rodziny protokolow IPv4
     ...
     */
    queue_num = 5;
	/* Argumenty: h = uchwyt polaczenia, queue_nume = numer kolejki,
				  &callback = unkcja callback wywolywana dla zakolejkowanego pakietu,
				  NULL = dane uzytwkonika przekazywane do funkcji callback) */
    ...
    /* Tryb kopiowania danych z kolejki do przestrzeni uzytkownika
	: (pakiet + meta-dane) i rozmiar pakietu.
	Argumenty: qh = uchwyt kolejki zwrocony przez create(),
			   mode = tryb kopiowania,
			   0xffff = dopusczalny rozmiar pakietu*/
    ...
    /* Obieranie pakietow: */
	// Caly pakiet jest odbierany za pomoca recv(), bo Netlink jest progkolem datagramowym
	// Argumenty: fd = dekstyptor gniazda, pozniej bufor na dane i jego rozmiar na koncu flagi chyba
    while ((rv = recv(fd, buf, sizeof(buf), 0)) >= 0) {
        fprintf(stdout, "Packet received. Entering callback.\n");
        /* Wywolanie funkcji callback i przekazanie jej pakietu: */
		/* Argumenty: h - uchwyt polaczenia, bufor zawierajacy pakiet,
		rv = rozmiar pakietu w buforze, czyli liczba bajtow zwrona przez recv() */
...
static int callback(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
     ...
     */
	/* Argumenty: gh = uchwyt kolejki, id = identyfikator przypisany do pakietu przez ntohl,
				  verdict = DROP lub ACCEPT, data_len = liczba bajtow w buforze, jezeli byla modyfikacja,
				  buf = bufor zawierajacy zmodyfikowany pakiet
	 */
...
static u_int32_t print_packet(struct nfq_data *tb) {
	...
	/* Przyjmuje wskaznik na strukture nfq_data, czyli uchwyt danych */
	/* Zwraca meta-naglowek odpowiadajacy przetwarzanemu pakietowi */
    ph = nfq_get_msg_packet_hdr(tb);
    if (ph) {
        /* ID pakietu w porzadku sieciowym: */
		/* Converts the unsigned integer netlong from network byte order to host byte order. */
		/* Netlong to host long */
