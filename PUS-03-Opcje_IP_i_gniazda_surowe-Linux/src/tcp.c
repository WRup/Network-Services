...
#define SOURCE_ADDRESS "192.0.2.4"
...
int main(int argc, char** argv) {
    ...
    /* Rozmiar okna TCP: */
    tcp_header->th_win              =       htons(sizeof(struct tcphdr));
    /* Numer sekwencyjny: */
    tcp_header->th_seq 				= 		htons(1);
    /* Acknowlegde number: */
    tcp_header->th_ack 				= 		0;
    /* Pole wyzerowane (unused): */
    tcp_header->th_x2 				= 		0;
    /* data offset 5B*32b = 20B */
    tcp_header->th_off 				= 		5;
    /* TCP flags: */
    ...
    pseudo_header->length           =       tcp_header->th_win;
    ...
    for (;;) {
	/*Losowa zmiana portu zrodla: */
        tcp_header->th_sport = htons(rand()%48127+1024);

	/* Ponowne obliczanie sumy kontrolnej spowodowane zmiana portu */
	tcp_header->th_sum =   internet_checksum(
                                                (unsigned short *)tcp_header,
                                                sizeof(struct tcphdr)
                                                + sizeof(struct phdr)
                                            );
