...
int main(int argc, char** argv) {
	...
	/* Ustawienie ilosci strumieni przychodzacych i wychodzacych klienta */
	...
	stream = 0;
    while(1){
		printf("Wiadomosc wysylana przez strumien: %d\n", stream);
		printf("Wiadomosc o tresci: ");
        memset(buffer, 0, sizeof(buffer));
        retptr = fgets(buffer, BUFF_SIZE, stdin);
        if ((retptr == NULL) || (strcmp(buffer, "\n") == 0)) {
            break;
        }
        retval = sctp_sendmsg (sockfd, buffer, strlen(buffer),
                result->ai_addr,result->ai_addrlen, 0, 0, stream, 0, 0);
        if (retval == -1) {
             perror("sctp_sendmsg()");
             exit(EXIT_FAILURE);
        }
        ...
        memset(buffer, 0, sizeof(buffer));
        retval = sctp_recvmsg(sockfd, (void *) buffer, sizeof (buffer),
